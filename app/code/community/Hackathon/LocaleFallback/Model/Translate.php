...
class Hackathon_LocaleFallback_Model_Translate extends Mage_Core_Model_Translate
{
    ...
    public function getTemplateFile($file, $type, $localeCode=null)
    {
        ...
        if (!file_exists($filePath)) { // If template doesn't exist, use fallback
            $filePath = Mage::getBaseDir('locale') . DS
                . Mage::getStoreConfig('general/locale/code_fallback')
                . DS . 'template' . DS . $type . DS . $file;
        }

        if (!file_exists($filePath)) { // If fallback doesn't exist, use store default
            $filePath = Mage::getBaseDir('locale') . DS
                . Mage::app()->getLocale()->getDefaultLocale()
                . DS . 'template' . DS . $type . DS . $file;
        }

        if (!file_exists($filePath)) {  // If store default doesn't exist, use en_US
            $filePath = Mage::getBaseDir('locale') . DS
